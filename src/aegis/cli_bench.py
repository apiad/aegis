...
app = typer.Typer(
    help="Benchmark the TUI against a real daemon and client.", no_args_is_help=True
)
_console = Console()
...
def run_cmd(
    scenario: list[str] = typer.Option(
        None, "--scenario", "-s", help="Scenario name (repeatable)."
    ),
    quick: bool = typer.Option(False, "--quick", help="A fast subset, one repeat."),
    repeat: int = typer.Option(3, "--repeat", min=1),
    size: str = typer.Option("120x40", "--size", help="COLSxROWS."),
    target: str = typer.Option(
        None, "--target", help="X.Y.Z from PyPI, or a python path."
    ),
    speed: float = typer.Option(1.0, "--speed", help="Replay speed factor."),
    profile: bool = typer.Option(
        False, "--profile", help="Record a py-spy speedscope profile."
    ),
    sabotage: int = typer.Option(
        0, "--sabotage", help="Sleep MS in every streaming paint."
    ),
    save: bool = typer.Option(
        False, "--save", help="Copy the summary to bench/history."
    ),
    out: Path = typer.Option(None, "--out", help="Runs directory."),
    keep: bool = typer.Option(
        False, "--keep", help="Keep the /tmp worlds for inspection."
    ),
) -> None:
    ...
    if unknown:
        raise typer.BadParameter(
            f"unknown scenario(s): {unknown}; see `aegis bench list`"
        )
...
def list_cmd() -> None:
    ...
    for s in SCENARIOS.values():
        t.add_row(
            s.name,
            "yes" if s.name in DEFAULT else "",
            "yes" if s.name in QUICK else "",
            s.description,
        )
    _console.print(t)
    base = runs_dir()
...
def compare_cmd(
    ...
    baseline: str = typer.Option(
        None, "--baseline", help="'latest-release' or a summary."
    ),
    force: bool = typer.Option(False, "--force", help="Compare across hosts or sizes."),
    all_rows: bool = typer.Option(
        False, "--all", help="Include noise and unchanged rows."
    ),
) -> None:
    """Compare two runs metric by metric; exit 1 if anything regressed."""
    from aegis.bench import BenchError
    from aegis.bench.compare import compare, latest_release, load_summary, min_repeats
    ...
    if thin < 3:
        _console.print(
            f"[yellow]note:[/] {thin} repeat(s) on the thinner "
    ...
    busy = [
        ...
        for s in (left, right)
        if any(
            sc.get("median", {}).get("host.cpu_busy_pct", 0) > BUSY_CPU_PCT
            for sc in s["scenarios"].values()
        )
    ]
    if busy:
        _console.print(
            f"[yellow]note:[/] {', '.join(busy)} ran on a busy "
    ...
    print_compare(
        rows,
        _console,
        a_label=left.get("run_id", "a"),
        b_label=right.get("run_id", "b"),
        all_rows=all_rows,
    )
    regressed = any(r.verdict == "regressed" for rs in rows.values() for r in rs)
...
def history_cmd(
    ...
    base = history_dir()
    files = (
        sorted(base.glob("*.json"), key=lambda p: (version_key(p) or (0, 0, 0), p.stem))
        if base.exists()
        else []
    )
    if not files:
        ...
        return
    print_history(
        [json.loads(p.read_text()) for p in files],
        list(metric or _HISTORY_METRICS),
        scenario,
        _console,
    )
...
def record_cmd(
    partial: bool = typer.Option(
        False, "--partial", help="Record with --include-partial-messages."
    ),
    prompt: str = typer.Option(
        None, "--prompt", help="Replace the default recording prompt."
    ),
    out: Path = typer.Option(
        None, "--out", help="Fixture path; defaults to the packaged one."
    ),
    ...
    try:
        n = record_fixture(
            dest, partial=partial, prompt=prompt or RECORD_PROMPT, model=model
        )
    except BenchError as exc:
        _console.print(f"[red]{exc}[/]")
        raise typer.Exit(2) from exc
...
def selftest_cmd(
    sabotage: int = typer.Option(
        ...
        "--sabotage",
        min=10,
        help="Milliseconds added to every streaming paint in the second run.",
    ),
) -> None:
    ...
    try:
        plain, _ = run(RunOptions(scenarios=["selftest-stream"], repeat=1), _console)
        slow, _ = run(
            RunOptions(scenarios=["selftest-stream"], repeat=1, sabotage_ms=sabotage),
            _console,
        )
    except BenchError as exc:
        _console.print(f"[red]bench failed:[/] {exc}")
    ...
    _console.print(
        f"marker p50 {lat:+.1f} ms (needs >= {need_lat:.0f}); "
