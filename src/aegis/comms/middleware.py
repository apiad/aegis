...
from aegis.comms.descriptors import aegis_family, aegis_target, descriptor_for
...
def _thread(result_data: object, args: dict, call_id: str) -> str:
    for source in (result_data if isinstance(result_data, dict) else {}, args):
...
class CommsMiddleware(Middleware):
    ...
    def _from(self, args: dict) -> str:
        ...
        return resolve_caller(self._tokens) or str(args.get("from_handle") or "")
