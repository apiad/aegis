...
def _agent_choices(bridge) -> list:
    ...
    for name in bridge.list_agents():
        ...
        if a is None:
            ...
            continue
        perm = (
            getattr(getattr(a, "permission", ""), "value", getattr(a, "permission", ""))
            or "?"
        )
        out.append(
            (
                name,
                f"{getattr(a, 'harness', '?')} · {getattr(a, 'model', '?')} · {perm}",
            )
        )
    return out
...
async def _sessions(ctx: CommandContext, args) -> CommandResult:
    ...
    lines = [
        f"{'*' if s.active else ' '} {s.handle} · {s.agent_slug} · {s.state}"
        for s in sessions
    ]
...
async def _agents_add(ctx: CommandContext, args) -> CommandResult:
    ...
    if not (slug and harness and model):
        return CommandResult(
            False,
            "usage: /agents add <slug> <harness> <model> [--effort E] [--permission P]",
        )
    ...
    return CommandResult(
        True, f"agent {slug} added", f"{harness} · {model} · persisted + hot-registered"
    )
...
async def _agents_remove(ctx: CommandContext, args) -> CommandResult:
    ...
    return CommandResult(
        True,
        f"agent {slug} removed",
        "persisted to .aegis.yaml; restart to drop the live profile",
    )
...
async def _spawn(ctx: CommandContext, args) -> CommandResult:
    ...
    if agent not in agents:
        return CommandResult(
            False, f"unknown agent: {agent}", "available: " + ", ".join(agents)
        )
    model = args.get("model")
    effort = args.get("effort")
...
async def _btw(ctx: CommandContext, args) -> CommandResult:
    ...
    if not prompt:
        return CommandResult(
            False, "usage: /btw <question>", "a bare /btw is a typo, not a request"
        )
    note = await ctx.bridge.side_note(ctx.handle, prompt)
    if not note.ok:
        return CommandResult(False, "side note failed", note.error or "no answer")
    body = [note.footer] if note.footer else []
    if note.needs_more:
        body.append(
            f"answered from {note.header} — `/fork` if you want it to actually go look."
        )
    if (
        getattr(note, "billed_to_session_profile", False)
        and ctx.handle not in _WARNED_UNSET
    ):
        _WARNED_UNSET.add(ctx.handle)
        body.append(
            "`text_generation:` is unset in .aegis.yaml — this side "
...
async def _recap(ctx: CommandContext, args) -> CommandResult:
    ...
    if not recap.ok:
        return CommandResult(False, "recap failed", recap.error or "no answer")
    from dataclasses import asdict
...
def _peer_targets(bridge) -> list:
    ...
    """
    return [
        (s.handle, f"{s.agent_slug} · busy" if s.state != "ready" else s.agent_slug)
...
async def _peer(ctx: CommandContext, args) -> CommandResult:
    ...
    prompt = args.get("prompt")
    if not target:
        return CommandResult(False, "usage: /peer <handle> <question>", "which peer?")
    if not prompt:
        return CommandResult(
            False,
            "usage: /peer <handle> <question>",
            "a peer ask with no question is a typo",
        )
    answer = await ctx.bridge.peer_ask(
        ctx.handle, target, prompt, cc=bool(args.get("cc"))
    )
    if not answer.ok:
        return CommandResult(
            False, f"@{target} could not answer", answer.error or "no answer"
        )
    from dataclasses import asdict
    ...
    return CommandResult(
        ...
        answer.answer,
        answer.footer,
        effect={"kind": "peer_answer", "answer": asdict(answer)},
    )
...
async def _queue(ctx: CommandContext, args) -> CommandResult:
    ...
    if sub is None:  # bare /queues → list
        ...
        for n in names:
            ...
            if q is None:
                ...
                lines.append(
                    f"  {n} · {q.agent_profile} · max_parallel {q.max_parallel}"
                )
        plural = "" if len(names) == 1 else "s"
    ...
    if args.flags.get("ephemeral"):
        ...
        return CommandResult(
            True, f"queue {name} created (ephemeral)", f"agent {agent} · max_parallel 1"
        )
    ...
    return CommandResult(
        True, f"queue {name} created", f"agent {agent} · persisted to .aegis.yaml"
    )
...
for _cmd in (
    ...
    SlashCommand(
        ...
        spec=ArgSpec(
            positionals=(
                ...
                Arg(
                    "harness", required=False, completer=tuple(sorted(_VALID_PROVIDERS))
                ),
    ...
    SlashCommand(
        "spawn",
        "start a new agent, from where you're standing",
    ...
    SlashCommand(
        "fork",
        "branch this conversation into a new tab",
        "/fork [prompt] [--slug S] [--model M] [--effort E]",
        _fork,
        spec=ArgSpec(
            positionals=(Arg("prompt", required=False, greedy=True),),
            flags=(Flag("slug"), Flag("model"), Flag("effort")),
        ),
    ),
    SlashCommand(
        "btw",
        "answer a side question from the recent window",
        "/btw <question>",
        _btw,
        spec=ArgSpec(positionals=(Arg("prompt", required=False, greedy=True),)),
    ...
    SlashCommand(
        "peer",
        "ask an idle peer, from where you're standing",
        "/peer <handle> [--cc] <question>",
        _peer,
        spec=ArgSpec(
            positionals=(
                Arg("handle", required=False, completer=_peer_targets),
                Arg("prompt", required=False, greedy=True),
            ),
    ...
    SlashCommand(
        "enqueue",
        "drop a task on a queue",
        "/enqueue <queue> <payload>",
        _enqueue,
        spec=ArgSpec(
            positionals=(
                Arg("queue", completer=lambda b: b.queue_manager.list_queues()),
