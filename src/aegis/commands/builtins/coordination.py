...
async def _groups(ctx: CommandContext, args) -> CommandResult:
    ...
    if sub in (None, "list"):
        ...
        lines = [
            f"  {r['name']} · {r['members']} member{'' if r['members'] == 1 else 's'}"
            for r in rows
        ]
        return CommandResult(
            True, f"{len(rows)} group{'' if len(rows) == 1 else 's'}", "\n".join(lines)
        )
    name = args.get("name")
    if not name:
    ...
    if sub == "status":
        ...
        members = (
            ", ".join(f"{m['handle']}({m['profile']})" for m in st.get("members", []))
            or "none"
        )
...
async def _schedules(ctx: CommandContext, args) -> CommandResult:
    ...
    if sub in (None, "list"):
        rows = list_payload(
            getattr(b, "scheduler", None), b.state_root, b.inline_schedule_names()
        ).get("schedules", [])
        if not rows:
            return CommandResult(True, "no schedules")
        lines = [
            f"  {'●' if r.get('enabled') else '○'} {r['name']} · "
            f"next {r.get('next_fire', '?')}"
            for r in rows
        ]
        return CommandResult(
            True,
            f"{len(rows)} schedule{'' if len(rows) == 1 else 's'}",
            "\n".join(lines),
        )
    name = args.get("name")
    if not name:
    ...
    if sub == "show":
        p = show_payload(
            getattr(b, "scheduler", None), b.state_root, b.inline_schedule_names(), name
        )
        if p is None:
            return CommandResult(False, f"schedule {name} not found")
        return CommandResult(
            True, f"schedule {name}", "\n".join(f"{k}: {v}" for k, v in p.items())
        )
    ...
    if sub == "remove":
        ...
        return CommandResult(
            False, f"cannot remove {name}", getattr(r, "status", "error")
        )
    if sub == "logs":
        ...
        return CommandResult(
            True, f"schedule {name} · {len(recs)} records", "\n".join(lines)
        )
    return CommandResult(
        False, "usage: /schedules [show|enable|disable|remove|logs <name>]"
    )
...
for _cmd in (
    SlashCommand(
        "groups",
        "list groups, or status/dissolve one",
        "/groups [status|dissolve <name>]",
        _groups,
        spec=ArgSpec(
            positionals=(
                Arg(
                    "subverb", required=False, completer=("list", "status", "dissolve")
                ),
                Arg(
                    "name",
                    required=False,
                    completer=lambda b: [g["name"] for g in b.groups.list_groups()],
                ),
            )
        ),
    ),
    SlashCommand(
        "schedules",
        "list schedules, or show/enable/disable/remove/logs one",
        "/schedules [show|enable|disable|remove|logs <name>]",
        _schedules,
        spec=ArgSpec(
            positionals=(
                Arg(
