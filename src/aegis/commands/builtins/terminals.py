...
async def _terminals(ctx: CommandContext, args) -> CommandResult:
    ...
    if sub in (None, "list"):
        ...
        return CommandResult(
            True,
            f"{len(infos)} terminal{'' if len(infos) == 1 else 's'}",
            "\n".join(lines),
        )
    name = args.get("name")
    if not name:
    ...
    if sub == "new":
        ...
        return CommandResult(
            True, f"terminal {name} started", f"pid {info.pid} · {info.shell}"
        )
    if sub == "close":
        await tm.close(name)
...
for _cmd in (
    SlashCommand(
        "terminals",
        "list terminals, or new/run/close one",
        "/terminals [new <name> | run <name> <cmd> | close <name>]",
        _terminals,
        spec=ArgSpec(
            positionals=(
                Arg(
                    "subverb", required=False, completer=("list", "new", "run", "close")
                ),
                Arg(
                    "name",
                    required=False,
                    completer=lambda b: [i.name for i in b.terminal_manager.list()],
                ),
