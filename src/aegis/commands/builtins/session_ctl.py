...
async def _themes(ctx: CommandContext, args) -> CommandResult:
    ...
    if full is None:
        return CommandResult(
            False, f"unknown theme: {name}", "available: " + ", ".join(THEME_NAMES)
        )
    return CommandResult(
        True, f"theme → {full}", effect={"kind": "theme", "name": full}
    )


async def _clear(ctx: CommandContext, args) -> CommandResult:
    return CommandResult(True, "transcript cleared", effect={"kind": "clear"})
...
async def _tasks(ctx: CommandContext, args) -> CommandResult:
    ...
    return CommandResult(True, "task dock toggled", effect={"kind": "tasks"})
...
async def _title(ctx: CommandContext, args) -> CommandResult:
    ...
    if not text:
        ...
        if regen is None:
            return CommandResult(
                False,
                "cannot regenerate a title in this frontend",
                "use `/title <text>`, or `/title --clear`",
            )
        res = await regen(ctx.handle)
        if isinstance(res, dict) and res.get("error"):
            return CommandResult(False, "could not regenerate the title", res["error"])
    ...
    return (
        CommandResult(True, f"title → {stored}")
        if stored
...
async def _reconnect(ctx: CommandContext, args) -> CommandResult:
    ...
    if reconnect is None:
        return CommandResult(False, "reconnect is not available in this frontend")
    try:
...
for _cmd in (
    ...
    SlashCommand(
        "close",
        "close the current or a named session",
        "/close [handle]",
        _close,
        spec=ArgSpec(
            positionals=(
    ...
    SlashCommand(
        "themes",
        "list themes, or switch to one",
        "/themes [name]",
        _themes,
        spec=ArgSpec(positionals=(Arg("name", required=False, completer=THEME_NAMES),)),
    ),
    SlashCommand("clear", "clear the visible transcript (cosmetic)", "/clear", _clear),
    SlashCommand("tasks", "show or hide the task dock (also F3)", "/tasks", _tasks),
):
    register(_cmd)
