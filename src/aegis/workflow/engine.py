...
class WorkflowEngine:
    ...
    def log(self, message: str) -> None:
        ...
        print(f"[workflow:{self.name}] {message}", file=sys.stderr, flush=True)
        if self._state_dir is None:
            return
        path = Path(self._state_dir) / "workflows" / f"{self.workflow_id}.jsonl"
        ...
        *,
        ...
        try:
            ...
            try:
                ...
                pass
            raise WorkflowError(f"bash timed out after {timeout}s: {cmd}")
        return _BashResult(
            args=cmd,
    ...
    async def enqueue(self, queue: str, payload: str, *, callback: bool = False) -> str:
        ...
        try:
            ...
            raise WorkflowError(f"unknown queue: {e.args[0]!r}") from e
    ...
    async def delegate(self, queue: str, payload: str) -> str:
        ...
        try:
            try:
                result = self._queue.enqueue(
                    queue, payload, enqueued_by=handle, callback=True
                )
            except KeyError as e:
                raise WorkflowError(f"unknown queue: {e.args[0]!r}") from e
            if isinstance(result, dict):
            ...
            msg = await promise
            if msg.status == "error":
                raise WorkflowError(f"task {task_id} failed: {msg.body}")
            return msg.body
        finally:
        ...
        *,
        ...
        try:
            if runner is not None and hasattr(runner, "spawn_subagent"):
                h = await runner.spawn_subagent(profile, alias=requested, **place)
            else:
                h = await self._bridge.spawn(profile, handle=requested, **place)
    ...
    async def close(self, handle: str) -> None:
        ...
        if self.host is not None and handle == self.host:
            raise ValueError(f"cannot close host handle: {handle!r}")
    ...
    # ── send ─────────────────────────────────────────────────────────
    async def send(
        self, handle: str, prompt: str, *, timeout: float | None = None
    ) -> str:
        ...
        if self._inbox is not None:
            msg = InboxMessage(
                sender=f"workflow:{self.name}", timestamp=self._now(), body=prompt
            )
    ...
    async def ask_human(
        ...
        *,
        ...
        if runner is None or not hasattr(runner, "register_human_question"):
            raise RuntimeError("ask_human: bridge has no workflow_runner")
...
class _EphemeralGroupHandle:
    ...
    async def broadcast(self, **kw) -> str:
        return await self.runtime.broadcast(self.name, sender="workflow", **kw)
...
def _attach_group_methods(cls):
    ...
    async def broadcast(
        self, group, *, objective, output_format, tool_guidance, boundaries
    ):
