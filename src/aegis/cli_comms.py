...
comms_app = typer.Typer(
    add_completion=False, help="Inspect the inter-agent call ledger."
)
...
def list_calls(
    ...
    thread: str = typer.Option(None, "--thread", help="Only calls on this thread id."),
    family: str = typer.Option(
        None, "--family", help="conversation | coordination | introspection | admin"
    ),
    ...
    rows = filter_rows(
        ledger.read_all(), handle=handle, thread=thread, family=family, since_iso=since
    )
    if not rows:
