...
def render_aegis_yaml(config: WizardConfig) -> str:
    """Produce the contents of a ``.aegis.yaml`` file from a WizardConfig.

    Hand-rolled YAML rather than ruamel.yaml so the header comment and
    block ordering stay stable across runs and the diff is review-able.
    """
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")

    lines = [
        "# .aegis.yaml — Aegis configuration",
    ...
    if config.queues:
        ...
        for q in config.queues:
            ...
            lines.append(f"    agent: {q.agent_slug}")
            lines.append(f"    max_parallel: {q.max_parallel}")
        lines.append("")
