...
class LoopVerdict(BaseModel):
    ...
    reason: str = Field(description="one sentence, addressed to the operator")
    addendum: str = Field(
        default="",
        description="only when continuing: "
...
async def judge(
    *,
    ...
    state = [
        f"The looping instruction, verbatim: {instruction}",
    ...
    if advisory:
        ...
        state.append(
            f"The agent asked to stop. Its claim, which you are "
