...
BRIEFING = (
    ...
    "Pick ONE wake mechanism per process: either run it foreground and arm a "
    "monitor, OR launch it as a native background task (Claude "
    "run_in_background) and let the task-completion notification wake you — "
    "do NOT do both (a monitor whose `done` watches a native background "
    "task's output file races that task's own notification).\n"
    ...
    "  - aegis_config_add_agent(slug, harness, model, effort?, permission?) : add a new "
    "agent profile. Hot-registers — the new slug is available to subsequent enqueue / spawn "
    "calls without restart.\n"
...
def build_server(bridge: AppBridge) -> FastMCP:
    ...
    async def aegis_schedule_push(name: str, spec: dict, from_handle: str,
        ...
        On success, returns ``{name, written_to}`` (local) or whatever
