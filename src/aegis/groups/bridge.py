...
class _GroupsBridge:
    ...
    async def spawn(
        ...
        return await self.wiring.spawn(profile=profile, group=group, handle=handle)
    ...
    async def wait_all(
        ...
        return await self.runtime.wait_all(group, timeout=timeout, reducer=reducer)
    ...
    def list_groups(self) -> list[dict]:
        ...
        return [{"name": n, "members": len(reg.get(n).members)} for n in reg.names()]
    ...
    async def status(self, group: str) -> dict:
        ...
        return {
            "name": g.name,
            "members": [
                {"handle": h, "profile": m.profile} for h, m in g.members.items()
            ],
            "current_broadcast": (
                {
                    "id": rec.id,
    ...
    async def move_member(self, handle: str, *, from_group: str, to_group: str) -> dict:
...
def make_groups_bridge(
    *, session_manager, inbox_router, state_dir: Path | None = None
) -> _GroupsBridge:
    ...
    runtime = GroupRuntime(
        registry=registry, inbox=inbox_router, member_bus=bus, log=log
    )
    wiring = GroupWiring(
    ...
    if log is not None:
        live = (
            set(session_manager.live_handles())
            if hasattr(session_manager, "live_handles")
            else set()
        )
