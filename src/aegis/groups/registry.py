...
class GroupRegistry:
    ...
    def create(self, name: str, *, sender: str = "system", at: str = "") -> Group:
    ...
    def add_member(
        self, group: str, ref: MemberRef, *, sender: str = "system", at: str = ""
    ) -> None:
        ...
        self._emit(
            group, event_member_added(ref.handle, ref.profile, sender, at or now_iso())
        )

    def remove_member(
        self, group: str, handle: str, *, reason: str = "closed-by-user", at: str = ""
    ) -> None:
        g = self.get(group)
        if handle in g.members:
            g.members.pop(handle)
            self._emit(group, event_member_removed(handle, reason, at or now_iso()))
        if not g.members:
            self._groups.pop(group, None)
            self._emit(group, event_dissolved("empty", at or now_iso()))

    def move_member(self, handle: str, *, from_group: str, to_group: str) -> None:
        ref = self.get(from_group).members[handle]
        self.add_member(to_group, ref)
        self.remove_member(from_group, handle)

    def dissolve(self, group: str, *, reason: str = "dissolved", at: str = "") -> None:
        ...
        for group in self._log.all_groups():
            ...
            for handle in list(members):
                if handle not in live_handles:
                    ...
                    self._emit(
                        group,
                        event_member_removed(handle, "lost-on-restart", now_iso()),
                    )
            if not members:
                self._groups.pop(group, None)
    ...
    def _in_flight_broadcasts(records: list[dict]) -> list[str]:
        started = {
            r["broadcast_id"] for r in records if r["kind"] == "broadcast_started"
        }
        completed = {
            r["broadcast_id"] for r in records if r["kind"] == "broadcast_completed"
        }
