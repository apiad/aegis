...
def _check_auth(request: Request, spec: RemotePlaneSpec) -> dict | None:
    ...
    if spec.accept_from:
        ...
        if peer not in spec.accept_from:
            return {"error": f"source ip {peer!r} not in accept_from", "_status": 403}
    if spec.accept_tokens:
        ...
        if token not in spec.accept_tokens:
            return {"error": "missing or invalid bearer token", "_status": 401}
    return None
...
def build_plane(bridge, spec: RemotePlaneSpec) -> Starlette:
    ...
    async def enqueue(request: Request) -> JSONResponse:
        ...
        try:
            ...
            return JSONResponse(
                {"error": f"unknown queue {e.args[0]!r}"}, status_code=404
            )
    ...
    async def budget_list(request: Request) -> JSONResponse:
        ...
        for name, q in qm._queues.items():
            if not q.budgets:
                ...
                continue
            tail = qm._load_recent_jsonl(name, max_age=max(b.window for b in q.budgets))
            d = evaluate_budgets(tail, q.budgets, now)
            if d.allowed:
                tightest = min(
                    d.checks, key=lambda c: (c.headroom / c.limit) if c.limit > 0 else 0
                )
                binding = (
                    f"${tightest.spent} of ${tightest.limit} / {tightest.window_str}"
                ...
                c = d.blocked_by[0]
                binding = (
                    f"${c.spent} of ${c.limit} / {c.window_str}"
                    if c.constraint == "usd"
                    else f"{c.spent} of {c.limit} {c.constraint} / {c.window_str}"
                )
                rows.append(
                    {
                        "name": name,
    ...
    async def budget_show(request: Request) -> JSONResponse:
        ...
        def _ser(c):
            return {
                ...
                "unblock_at": c.unblock_at.isoformat().replace("+00:00", "Z")
                if c.unblock_at
                else None,
            }
    ...
    if is_bridge:
        ...
        routes.append(Route("/remote/v1/schedule", schedule_list, methods=["GET"]))
        routes.append(
            Route("/remote/v1/schedule/{name}", schedule_show, methods=["GET"])
        )
        routes.append(
            Route("/remote/v1/schedule/{name}", schedule_remove, methods=["DELETE"])
        )
        routes.append(
            Route("/remote/v1/schedule/{name}/logs", schedule_logs, methods=["GET"])
        )
    routes.append(Route("/remote/v1/budget", budget_list, methods=["GET"]))
    routes.append(Route("/remote/v1/budget/{queue}", budget_show, methods=["GET"]))
...
def run_plane_async(app: Starlette, bind: str) -> asyncio.Task:
    ...
    config = uvicorn.Config(
        app, host=host, port=int(port_s), log_level="info", access_log=False
    )
