...
def print_run(summary: dict, console: Console) -> None:
    ...
    console.print(
        f"[bold]{summary['run_id']}[/]  {fp['aegis_build']} "
    ...
    for name, sc in summary["scenarios"].items():
        ...
        for metric, value in sc.get("median", {}).items():
            ...
            t.add_row(
                metric, _fmt(value), _unit(metric), " ".join(_fmt(r) for r in reps)
            )
        console.print(t)
        bad = sorted(
            {g["name"] for rep in sc.get("gates", []) for g in rep if not g["ok"]}
        )
        if bad:
...
def render_markdown(summary: dict) -> str:
    ...
    for name, sc in summary["scenarios"].items():
        ...
        if sc.get("median"):
            ...
            lines += [
                f"| {m} | {_fmt(v)} | {_unit(m)} |" for m, v in sc["median"].items()
            ]
            lines.append("")
    return "\n".join(lines)
...
def print_compare(
    ...
    *,
    ...
    for name, rows in rows_by_scenario.items():
        shown = [r for r in rows if all_rows or r.verdict not in ("noise", "same")]
        t = Table(title=name)
        ...
        if shown:
            ...
            console.print(f"{name}: no change beyond noise ({len(rows)} metrics)")


def print_history(
    summaries: list[dict], metrics: list[str], scenario: str, console: Console
) -> None:
    t = Table("build", "topology", *metrics, title=scenario)
    for s in summaries:
        med = s["scenarios"].get(scenario, {}).get("median", {})
        t.add_row(
            s["fingerprint"]["aegis_build"],
