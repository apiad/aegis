...
_LOREM = (
    "the compositor diffs spans against the previous frame and writes "
...
def synthetic_blocks(
    n: int, gap_ms: float, words: int = 12, mark: bool = True
) -> list[dict]:
    return [
        {
            "dt_ms": float(gap_ms),
        ...
        steps.append(
            {
                "dt_ms": 0.0,
                "mark": False,
                "line": _assistant_text(f"{_words(30, i)}\n\n", f"f{i}"),
            }
        )
        steps.append(
            {
...
def acp_chunks(
    n: int, rate_hz: float, mark_every: int = 5, mark: bool = True
) -> list[dict]:
    gap = 1000.0 / rate_hz
    steps = []
    for i in range(n):
...
def load_fixture(name: str) -> list[dict]:
    ...
    raw = (
        resources.files("aegis.bench")
        .joinpath("fixtures", f"{name}.jsonl")
