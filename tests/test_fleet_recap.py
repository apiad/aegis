...
from aegis.recap import StandingRecap, recap_in_flight

V = StandingRecap(task="Close the loop", outcome="Closing the loop on the watcher.",
                  next="", attention="waiting")
...
def test_a_good_call_returns_the_present_outcome(fake_agent):
    d = FakeDriver(V)
    r = asyncio.run(recap_in_flight(replay=[], facts=TurnFacts(), driver=d,
                                    agent=fake_agent, cwd="."))
    assert r.ok is True
    assert r.line == "Closing the loop on the watcher."
    assert r.task == "Close the loop"


def test_the_driver_is_handed_the_in_flight_schema_prompt_and_facts(fake_agent):
    """What is paid for is what reaches the driver: the one schema, the
    shared prompt plus the still-running addendum, and the FACTS block
    that guards against claiming work that did not happen."""
    from aegis.digest.models import CommitLine, RepoDelta
    from aegis.recap import IN_FLIGHT_ADDENDUM, SYSTEM
    ...
    assert instructions[0] == SYSTEM + IN_FLIGHT_ADDENDUM
...
async def test_recap_in_flight_for_bills_the_text_generation_agent(
    ...
    session's own model."""
    from aegis.recap import IN_FLIGHT_ADDENDUM, SYSTEM, recap_in_flight_for
...
def test_the_window_is_the_generous_one(fake_agent):
    ...
    floor, $0.0012. Squeezing it saves nothing and measurably degrades the
    answer."""
