...
# `test_the_local_tui_boot_gives_its_app_a_view_state` lived here. It
# asserted that the boot builds a ViewState and hands the app the same
# object it later persists, through `LocalTuiAttachment`. That class went
# with `--foreground`, and the property did not go with it: `open_view`
# builds the state now and `View.persist` saves it, which
# tests/views/test_view.py::test_the_app_holds_the_same_view_state_object
# asserts against the path that still exists.
