...
| `aegis_task_status(task_id)` | Inspect a previously enqueued task — useful when `callback=false` or you want to poll. Carries `worker_handle`, which is what makes a `recoverable` status actionable rather than just informative. |
| `aegis_task_resume(task_id)` | Put a **parked** queue worker back to work: its harness is rebuilt under the same session, with the conversation intact, and its retry budget starts over. Read it first with `aegis_read_peer(<worker_handle>)`. See [when a worker stalls](queues.md#when-a-worker-stalls). |
| `aegis_task_retry(task_id)` | Re-run a finished task from its **original payload**, as a new task. Kept separate from `aegis_task_resume` and never automatic: a worker that got halfway may already have committed, pushed or deployed, so re-running its prompt is a second execution rather than a recovery. |
...
| `aegis_repo_cost(repo, from_handle, refresh=false)` | What a repository cost to build: tokens priced at list rate and attributed to that repo by locality, plus its commits, assisted hours and transcript coverage. Reads the cache at `<state>/cost/<repo>.json` that `aegis usage repo --json` writes, and reports its age in hours, because recomputing reads thousands of transcript files and takes about a minute. `refresh=true` pays that minute and needs a path rather than a bare name. See [what a repo cost to build](usage.md#what-a-repo-cost-to-build). |
