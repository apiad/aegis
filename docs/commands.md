...
The contents of a collection are a subverb, not the singular noun: `/queues`
lists the queues you configured and `/queues tasks` lists the tasks on them.
`/queue` stays retired — it was the old name for `/queues`, and bringing it
back under any meaning puts a singular noun listing a collection beside
...
| `/recap` | Where this session stands — a task / outcome / next block over the last eight turns, at the level of what is being solved rather than which files changed. Never appended to the session log. Deferred — `Esc` cancels. See [Recaps](usage.md#recaps-and-the-loop-judge). |
...
| `/queues [tasks [<queue>] \| new <name> [agent] [--ephemeral]]` | List / create queues (persisted, or ephemeral in the live manager only). `tasks` lists queue **tasks** and their states instead — parked, in flight, waiting, then recent history — with full task ids, because that is what `/resume` takes. See [Queues](queues.md#when-a-worker-stalls). |
