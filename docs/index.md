...
- **Remote plane.** Run `aegis serve` on a second machine and an agent on the first can `aegis_enqueue(target="vps", …)` to hand long work off over HTTP. Trust anchor is the tailnet (Headscale / WireGuard); bearer-token and IP-allowlist gates compose with AND. The remote pings Telegram on completion. See [Remote plane](remote.md).
