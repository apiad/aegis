...
- **Plugin substrate.** Extend aegis without forking it: `@hook` for pre/post-turn + session lifecycle, `@tool` for FastMCP-registered helpers, `@workflow` for orchestrated procedures. `aegis plugin install/uninstall/list` over local paths or `gh:owner/repo#path` registry URLs; comment-preserving lockfile; rolls back on failure. Two canonical plugins ship in-tree — `skill-system` (Claude-Code-style skill selection on any harness) and `memory-system` (Hermes-inspired persistent memory with periodic dreaming). See [Plugins](plugins.md).
